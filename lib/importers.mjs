...
import { analyzeMaterial, needsSpokenFormAnalysis } from "./analysis.mjs";
import { captureAiSettings } from "./ai-settings.mjs";
...
export async function importLarkMaterial(materialId, sourceUrl, minuteToken, report, options = {}) {
  ...
  try {
    ...
    await addAnalysis(materialId, report, options);
...
export async function importLocalMaterial(materialId, report, options = {}) {
  const directory = materialDir(materialId);
  let material = await readMaterial(materialId);
...
export async function addAnalysis(materialId, report, options = {}) {
  const aiSettings = process.env.SKIP_CODEX_ANALYSIS === "1"
    ? null
    : options.aiSettings || await captureAiSettings();
  const directory = materialDir(materialId);
  ...
  try {
    ...
    const analyzed = await analyzeMaterial(material, directory, {
      ...
      onBatch: async (partial) => {
        await updateMaterial(materialId, (latest) => {
          ...
          latest.analysisProvider = partial.analysisProvider;
          latest.analysisStatus = "processing";
          latest.status = "ready";
    ...
    material = await updateMaterial(materialId, (latest) => {
      ...
      if (analyzed.analysisProvider) latest.analysisProvider = analyzed.analysisProvider;
      const stillPending = latest.sentences.some(needsSpokenFormAnalysis);
      latest.analysisStatus = stillPending ? "pending" : (analyzed.skipped ? "skipped" : "ready");
      latest.stage = stillPending ? "原文已修正，等待重新生成本句讲解" : "解析完成";
      latest.status = "ready";
      return latest;
    });
  } catch (error) {
    material = await updateMaterial(materialId, (latest) => {
      ...
      latest.stage = "精听材料已就绪，AI 讲解生成失败";
...
export function mergeSentenceAnalysis(currentSentences = [], analyzedSentences = []) {
  const analyzedById = new Map(analyzedSentences.map((sentence) => [sentence.id, sentence]));
  return currentSentences.map((sentence) => {
    const analyzedSentence = analyzedById.get(sentence.id);
    if (!analyzedSentence || analyzedSentence.text !== sentence.text) return sentence;
    return { ...sentence, analysis: analyzedSentence.analysis || null };
  });
}
