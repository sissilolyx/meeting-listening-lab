...
- Learning is learner-led after reveal. Any selected expression can become an in-context Codex question and, when useful, a review note bound to its exact natural sentence.
...
- Keep the data boundary explicit. Media, progress, and question history stay on the learner's computer and are never stored in the repository. Transcript excerpts are sent to the learner's own locally authenticated Codex only for requested analysis.
